...
install-utils:
	...
	find . -maxdepth 1 -perm +100 -type f -name 'debconf-*' | grep -v debconf-get-selections | grep -v debconf-show | grep -v debconf-copydb | \
...
install-rest:
	...
	find . -maxdepth 1 -perm +100 -type f -name debconf -or -name debconf-show -or -name debconf-copydb -or -name debconf-get-selections | \
		xargs -i install {} $(prefix)/usr/bin
