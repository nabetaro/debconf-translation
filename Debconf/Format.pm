...
Read one record from the filehandle, parse it, and return a structure as
required by Debconf::DbDriver::Cached. Note that the filehandle may contain
multiple records, so it must be able to recognize an end-of-record
delimiter of some kind and stop reading after it.
